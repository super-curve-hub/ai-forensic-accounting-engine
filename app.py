...
st.markdown("""
<style>
.main-title{font-size:2.1rem;font-weight:800;letter-spacing:-.03em;margin-bottom:.1rem}
.sub-title{color:#666;font-size:.95rem;margin-bottom:1.2rem}
.metric-card{border:1px solid rgba(49,51,63,.15);border-radius:18px;padding:18px;background:rgba(255,255,255,.65);box-shadow:0 1px 3px rgba(0,0,0,.04);margin-bottom:12px}
.metric-label{font-size:.78rem;color:#666;margin-bottom:8px}
.metric-value{font-size:1.55rem;font-weight:800;color:#111}
.small-note{font-size:.78rem;color:#777}
.grade-box{border-radius:18px;padding:18px;margin-top:8px;margin-bottom:16px;font-weight:700}
.grade-a{background:rgba(32,201,151,.12);border:1px solid rgba(32,201,151,.35);color:#087f5b}
.grade-b{background:rgba(116,192,252,.12);border:1px solid rgba(116,192,252,.35);color:#1864ab}
.grade-c{background:rgba(255,212,59,.16);border:1px solid rgba(255,212,59,.45);color:#8d6b00}
.grade-d{background:rgba(255,146,43,.14);border:1px solid rgba(255,146,43,.45);color:#b35c00}
.grade-f{background:rgba(250,82,82,.12);border:1px solid rgba(250,82,82,.45);color:#c92a2a}
</style>
...
USER_AGENT = "super-curve-hub research i.love.melonpan@gmail.com"
...
def grade_class(g):
    return {"A":"grade-a","B":"grade-b","C":"grade-c","D":"grade-d","F":"grade-f"}.get(g,"grade-c")
...
def run_forensic_engine(ticker, wacc=0.10):
    ...
    def series_for_tag(tag):
        ...
        if not {"form","unit","end","val"}.issubset(raw.columns):
            return pd.DataFrame(columns=["date", tag])
        tmp = raw[(raw["form"].isin(["10-Q","10-Q/A","10-K","10-K/A"])) & (raw["unit"] == "USD")].copy()
        if tmp.empty:
            return pd.DataFrame(columns=["date", tag])
        tmp["date"] = pd.to_datetime(tmp["end"], errors="coerce")
        tmp["filed"] = pd.to_datetime(tmp.get("filed"), errors="coerce")
    ...
    def resolve_metric(metric, tags):
        best, best_tag, best_count = None, None, -1
        for tag in tags:
            s = series_for_tag(tag)
            if s.empty: continue
            cnt = s[tag].notna().sum()
            if cnt > best_count:
                best, best_tag, best_count = s, tag, cnt
        if best is None:
    ...
    TAG_MAP = {
        ...
        "COGS":["CostOfRevenue","CostOfGoodsAndServicesSold","CostOfGoodsSold","CostOfGoodsSoldExcludingDepreciationDepletionAndAmortization","CostOfRevenueExcludingDepreciationAndAmortization"],
    ...
    master, used_tags = None, {}
    for metric, tags in TAG_MAP.items():
        s, used = resolve_metric(metric, tags)
        used_tags[metric] = used if used else "NOT FOUND"
        if s.empty: continue
    ...
    required = ["Revenue","COGS","OperatingIncome","PretaxIncome","IncomeTax","NetIncome","CFO","Capex","Assets","Liabilities","Equity","Cash","Receivables","Inventory","SBC","OperatingLeaseLiability","DeferredRevenue"]
    for c in required:
        if c not in df.columns:
            df[c] = np.nan

    for c in ["Cash","SBC","OperatingLeaseLiability","DeferredRevenue"]:
        df[c] = df[c].fillna(0)

    flows = ["Revenue","COGS","OperatingIncome","PretaxIncome","IncomeTax","NetIncome","CFO","Capex","SBC"]
    for c in flows:
        df[f"{c}_TTM"] = df[c].rolling(4, min_periods=4).sum()
    ...
    df["DSO"] = np.nan if df["Receivables"].isna().all() else safe_div(df["Receivables"], df["Revenue_TTM"] / 365)
    df["InventoryDays"] = np.nan if (df["Inventory"].isna().all() or df["COGS_TTM"].isna().all()) else safe_div(df["Inventory"], df["COGS_TTM"] / 365)

    scores, flags = [], []
    for _, r in df.iterrows():
        score, f = 0, []
        if pd.notna(r["AccrualRatio"]):
            if r["AccrualRatio"] > 0.10: score += 25; f.append("Accrual distortion severe")
            elif r["AccrualRatio"] > 0.05: score += 15; f.append("Accrual distortion moderate")
        if pd.notna(r["CFO_to_NI"]) and r["NetIncome_TTM"] > 0:
            if r["CFO_to_NI"] < 0.80: score += 20; f.append("Weak CFO conversion")
            elif r["CFO_to_NI"] < 1.00: score += 10; f.append("CFO below NI")
        if pd.notna(r["SBC_to_Revenue"]):
            if r["SBC_to_Revenue"] > 0.15: score += 20; f.append("High SBC burden")
            elif r["SBC_to_Revenue"] > 0.08: score += 10; f.append("Moderate SBC burden")
        if pd.notna(r["DSO"]):
            if r["DSO"] > 90: score += 15; f.append("High DSO")
            elif r["DSO"] > 60: score += 10; f.append("Moderate DSO")
        if pd.notna(r["InventoryDays"]):
            if r["InventoryDays"] > 120: score += 15; f.append("Inventory buildup")
            elif r["InventoryDays"] > 90: score += 10; f.append("Moderate inventory buildup")
        if pd.notna(r["ROIC_WACC_Spread"]) and r["ROIC_WACC_Spread"] < 0:
            score += 15; f.append("ROIC below WACC")
...
def render_grade(latest):
    ...
    css = grade_class(grade)
    st.markdown(f'<div class="grade-box {css}">Forensic Grade: {grade}<br>Regime: {latest.get("Regime","NA")}</div>', unsafe_allow_html=True)
...
def latest_row_for_table(result):
    l = result["latest"]
    return {
        "Ticker":l.get("Ticker"), "Company":l.get("Company"), "Grade":l.get("Grade"), "Regime":l.get("Regime"),
        "ROIC":l.get("ROIC_TTM"), "ROIC-WACC":l.get("ROIC_WACC_Spread"), "Accrual":l.get("AccrualRatio"),
        "CFO/NI":l.get("CFO_to_NI"), "SBC/Revenue":l.get("SBC_to_Revenue"), "DSO":l.get("DSO"),
        "InventoryDays":l.get("InventoryDays"), "Risk":l.get("ForensicRiskScore"), "Quality":l.get("QualityScore"),
        "Flags":l.get("Flags")
    }
...
def render_charts(result):
    ...
    with st.expander("ROIC Trend", expanded=True):
        st.plotly_chart(px.line(df, x="date", y="ROIC_pct", title="ROIC"), use_container_width=True)
    with st.expander("Accrual Trend"):
        st.plotly_chart(px.line(df, x="date", y="Accrual_pct", title="Accrual Ratio"), use_container_width=True)
    with st.expander("Forensic Risk Trend"):
        st.plotly_chart(px.bar(df, x="date", y="ForensicRiskScore", title="Forensic Risk Score"), use_container_width=True)
...
def render_data_tables(result):
    df = result["df"]
    cols = ["date","Revenue_TTM","COGS_TTM","NOPAT_TTM","ROIC_TTM","ROIC_WACC_Spread","EconomicEarnings_TTM","AccrualRatio","CFO_to_NI","FCF_to_NI","SBC_to_Revenue","DSO","InventoryDays","ForensicRiskScore","QualityScore","Flags"]
    ...
    with st.expander("Resolved XBRL Tags"):
        st.dataframe(pd.DataFrame([{"Metric":k,"ResolvedTag":v} for k,v in result["used_tags"].items()]), use_container_width=True)
...
with tab_analysis:
    ...
    with cb:
        analyze = st.button("Analyze", key="analysis_button", use_container_width=True)
    if analyze:
        try:
            with st.spinner(f"Analyzing {ticker}..."):
                result = run_forensic_engine(ticker, wacc=wacc)
            latest = result["latest"]
            st.markdown(f"### {latest.get('Company')} ({latest.get('Ticker')})")
            st.caption(f"CIK: {latest.get('CIK')} | WACC: {wacc_pct:.1f}%")
            render_grade(latest)
            render_kpi_cards(latest)
            render_charts(result)
            render_data_tables(result)
        ...
        for i,t in enumerate(selected):
            try:
                with st.spinner(f"Analyzing {t}..."):
                    rows.append(latest_row_for_table(run_forensic_engine(t, wacc=wacc)))
            except Exception as e:
...
with tab_screening:
    ...
    if st.button("Run Screen", key="screen_button"):
        ...
        if rows:
            ...
            screen_df = raw_df[
                (raw_df["ROIC"].fillna(-999) >= roic_min_pct/100) &
                (raw_df["Risk"].fillna(999) <= risk_max) &
                (raw_df["Accrual"].fillna(999) <= accrual_max_pct/100)
            ].copy()
...
st.caption("v2.1: card-centered UX + Watchlist + Compare + Screening. TTM uses pragmatic approximation; full quarterization remains future work.")
